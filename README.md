...
Consultar la lista de estudiantes de una sección, con sus datos de asistencia, tendencia de notas y contexto.
Calcular automáticamente un puntaje de 0 a 100 que indica el nivel de señal de bajo rendimiento (🟢 bajo · 🟡 medio · 🔴 alto).
Mostrar al tutor una lista priorizada, ordenada de mayor a menor señal.
Explicar el puntaje: qué parte viene de asistencia, de notas, de trabajo o de repitencia (no es una caja negra).
Generar, con un clic, una recomendación de acompañamiento redactada por un modelo de IA a partir de los datos reales del estudiante.
Propuesta de valor
...
El diagrama detallado (Mermaid) está en /docs/arquitectura.md.
...
Estado del proyecto — Fase 1
 Interfaz web inicial con panel de estudiantes y semáforo de señales.
 Base de datos conectada en Supabase con datos de prueba.
 Cálculo del puntaje de señal de bajo rendimiento (ponderado por asistencia, notas, trabajo y repitencia).
 Backend en FastAPI con endpoint de recomendación por IA (en integración con el frontend).
